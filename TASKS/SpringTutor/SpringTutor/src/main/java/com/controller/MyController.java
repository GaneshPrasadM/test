...
public class MyController {
	...
	public @ResponseBody List<ResponseObject> myController(@RequestBody VariableModule variableModule) throws DatatypeConfigurationException{
		
		
		return webservice(variableModule);
	}
	...
	public List<ResponseObject> webservice(VariableModule variableModule) throws DatatypeConfigurationException{
		
		List<ResponseObject> requestObjectset = new ArrayList<ResponseObject>();
     ...
     //   System.out.println(quote.getLocalRefId());
	        ...
	        _calculateSupplierQuote_supplierQuote.setLocalRefId(localRefId);
	        ...
	        _calculateSupplierQuote_supplierQuoteQuoteItemVal1.setQuoteItemID(QuoteItemID);
...
//	        functionSpecification2.setFunctionValueSpecification(value2);
	        ...
	        _calculateSupplierQuote_supplierQuoteOperatorQuoteQuoteItemSiteAddress.setApartmentNumber(buildingnumber);
	        ...
	        try {
	            ...
	            for (int i = 0; i < _calculateSupplierQuote__return.getQuoteItem().size(); i++) {
	    			...
	    			comments=_calculateSupplierQuote__return.getQuoteItem().get(i).getComments();
	    			requestObject.setComments(comments);
	    			...
	    			localrefenceceid=_calculateSupplierQuote__return.getQuoteItem().get(i).getLocalRefId();
	    			requestObject.setLocalrefenceceid(localrefenceceid);
	    			System.out.println("getQuoteItemID :"+_calculateSupplierQuote__return.getQuoteItem().get(i).getQuoteItemID());
	    			quoteitemid=_calculateSupplierQuote__return.getQuoteItem().get(i).getQuoteItemID();
	    			requestObject.setQuoteitemid(quoteitemid);
	    			...
	    			currency=_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getCurrency().getCurrencyCode();
	    			requestObject.setCurrency(currency);
	    			System.out.println("getAreaCovered :"+_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getCommitmentSpecification().getDurationValue());
	    			
		    	      
		    	     
		    	        contactduration=_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getCommitmentSpecification().getDurationValue();
		    	        requestObject.setContactduratio(contactduration);
	    			//wanted 2
	    			...
	    			technology1=_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getProductSpecificationType();
	    			requestObject.setTechnology1(technology1);
	    			...
	    			for (int j = 0; j <_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getFunctionSpecification().size(); j++) {
			    			...
			    			speed1=_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getFunctionSpecification().get(j).getFunctionValueSpecification().getFunctionValue();
			    			requestObject.setSpeed1(speed1);
	    			...
	    			for (int k = 0; k <_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getTariffSpecification().size(); k++) {
		    			...
		    			if(k==0){
			    			nrc_value = _calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getTariffSpecification().get(k).getTariffValueSpec().getAmount();
			    			requestObject.setNrc_value(nrc_value);
			    			}
			    			if(k==1){
				    			mrc_value = _calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getProductSpecification().getTariffSpecification().get(k).getTariffValueSpec().getAmount();
				    			requestObject.setMrc_value(mrc_value);
				    			}
	    			}
	    			...
	    			System.out.println("getOfferSpecification().getCatalogue().getOperator().getName() :"+_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getCatalogue().getOperator().getName());
	    			 provider=_calculateSupplierQuote__return.getQuoteItem().get(i).getOfferSpecification().getCatalogue().getOperator().getName();
	    			 requestObject.setProvider(provider);
	    			System.out.println("***************************");
	    			
	    			
	    	        country1=country;
	    	        requestObject.setCountry1(country1);
	    	        cityname1=cityname;
	    	        requestObject.setCityname1(cityname1);
	    	        address1=streetname;
	    	        requestObject.setAddress1(address1);
	    	        postalcode1=postalcode;
	    	        requestObject.setPostalcode1(postalcode1);
	    	        servicename1=servicename;
	    	        requestObject.setServicename1(servicename1);
	    	        
	    	       
	    	        requestObjectset.add(requestObject);
	    	        
	    			insertintoResponsetable(localrefenceceid,quoteitemid,country1,cityname1,address1,postalcode1,speed1,technology1,servicename1,mrc_value,nrc_value,currency,contactduration,provider,comments);
...
//            "jdbc:oracle:thin:@10.237.59.104:1521:MEERKAT","meerkat","meerkat");  
	...
	//For inserting on Meerkat Database Response
		public void insertintoResponsetable(String localrefenceceid,String quoteitemid,String country1,String cityname1,String address1,String postalcode1,String speed1,String technology1,String servicename1,double mrc_value,double nrc_value,String currency,double contactduration,String provider,String comments){
...
//	            String a="VALUES ('"+localrefenceceid+"','"+quoteitemid+"',CURRENT_TIMESTAMP,'"+country1+"','"+cityname1+"','"+address1+"','"+postalcode1+"','"+speed1+"','"+technology1+"','"+servicename1+"',"+mrc_value+","+nrc_value+",'"+currency+"','"+contactduration+"','"+provider+"','"+comments+"')"; 
//
...
//	            Statement st = myConnection.createStatement(); 
	            st.executeUpdate("INSERT INTO webalc2_response " + 
	                "VALUES ('"+localrefenceceid+"','"+quoteitemid+"',CURRENT_TIMESTAMP,'"+country1+"','"+cityname1+"','"+address1+"','"+postalcode1+"','"+speed1+"','"+technology1+"','"+servicename1+"',"+mrc_value+","+nrc_value+",'"+currency+"','"+contactduration+"','"+provider+"','"+comments+"')"); 
